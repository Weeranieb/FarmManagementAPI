...
func (_m *MockFarmRepository) WithTx(tx *gorm.DB) repository.FarmRepository {
	ret := _m.Called(tx)
	...
	var r0 repository.FarmRepository
	if rf, ok := ret.Get(0).(func(*gorm.DB) repository.FarmRepository); ok {
		r0 = rf(tx)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(repository.FarmRepository)
		}
	}
