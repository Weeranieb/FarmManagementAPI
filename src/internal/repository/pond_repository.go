...
SELECT
  ...
  ap.net_result AS ap_net_result, ap.total_fish AS ap_total_fish,
...
func (r *pondRepository) GetByIDWithFarmAndActivePond(ctx context.Context, pondId int) (*PondWithFarmAndActivePond, error) {
	...
	// No row means pond not found (INNER JOIN farm would still return a row if pond existed)
	if row.PondId == 0 {
		return nil, nil
	}
	pond := &model.Pond{
	...
	out := &PondWithFarmAndActivePond{Pond: pond, ClientId: row.ClientId}
	if row.ApId != nil {
