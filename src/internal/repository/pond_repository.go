...
SELECT
  ...
  ap.default_fresh_feed_collection_id AS ap_default_fresh_feed_collection_id,
  ap.default_pellet_feed_collection_id AS ap_default_pellet_feed_collection_id,
...
func rowToPondWithFarmAndActivePond(row *projection.PondFillQueryRow) *PondWithFarmAndActivePond {
	...
	if row.ApId != nil {
		...
		ap := &model.ActivePond{
			...
			FishTypes:                     fishTypes,
			DefaultFreshFeedCollectionId:  copyIntPtr(row.ApDefaultFreshFcId),
			DefaultPelletFeedCollectionId: copyIntPtr(row.ApDefaultPelletFcId),
		}
