...
| Setting              | Value           | Why                                                                 |
| -------------------- | --------------- | ------------------------------------------------------------------- |
| **Root Directory**   | `backend`       | So Vercel uses the folder that contains `go.mod` and `vercel.json`. If this is wrong, builds often fail at `go mod tidy` because `go.mod` is not found. |
...
`go.mod` should use **`go 1.24`** (no patch) so Vercel’s `@vercel/go` builder can select a supported toolchain. Pinning an arbitrary patch (e.g. `1.24.9`) can make the builder try to download that exact release and fail if it is unavailable.
...
**Optional upload paths** (on serverless, prefer external storage):
